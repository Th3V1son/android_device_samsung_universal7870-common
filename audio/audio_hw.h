...
/* TODO: remove resampler if possible when AudioFlinger supports downsampling
 * from 48 to 8 */
 ...
 * so if format = 16-bit PCM and channels = Stereo, frame size = 2 ch * 2 = 4
 * bytes DEEP_BUFFER_OUTPUT_PERIOD_SIZE = 1024 means 1024 * 4 = 4096 bytes We
 * should take care of returning proper size when AudioFlinger queries for the
 * buffer size of an input/output stream
 */
...
#define PLAYBACK_HDMI_MULTI_PERIOD_BYTES                                       \
  (PLAYBACK_HDMI_MULTI_PERIOD_SIZE *                                           \
   PLAYBACK_HDMI_MULTI_DEFAULT_CHANNEL_COUNT * 2)
...
struct stream_out {
  ...
  pthread_mutex_t
      pre_lock; /* acquire before lock to avoid DOS by playback thread */
  pthread_cond_t cond;
  struct pcm_config config;
  ...
  // echo_reference_generation indicates if the echo reference used by the
  // output stream is in sync with the one known by the audio_device. When
  // different from the generation stored in the audio_device the output stream
  // must release the echo reference. always modified with audio device and
  // stream mutex locked.
...
struct audio_device {
  ...
  struct echo_reference_itfe *echo_reference;
  // echo_reference_generation indicates if the echo reference used by the
  // output stream is in sync with the one known by the audio_device.
  // incremented atomically with a memory barrier and audio device mutex locked
  // but WITHOUT stream mutex locked: the stream will load it atomically with a
  // barrier and re-read it with audio device mutex if needed
