...
typedef enum {
  ...
  RIL_CDMA_SMS_DIGIT_MODE_MAX32 =
      0x10000000 /* Force constant ENUM size in structures */
} RIL_CDMA_SMS_DigitMode;

typedef enum {
  RIL_CDMA_SMS_NUMBER_MODE_NOT_DATA_NETWORK = 0,
  RIL_CDMA_SMS_NUMBER_MODE_DATA_NETWORK = 1,
  RIL_CDMA_SMS_NUMBER_MODE_MAX32 =
      0x10000000 /* Force constant ENUM size in structures */
...
typedef enum {
   ...
   * an ASCII character. Examples are "x@y.com,a@b.com - ref
  ...
  RIL_CDMA_SMS_NUMBER_TYPE_MAX32 =
      0x10000000 /* Force constant ENUM size in structures */
} RIL_CDMA_SMS_NumberType;

typedef enum {
  RIL_CDMA_SMS_NUMBER_PLAN_UNKNOWN = 0,
  RIL_CDMA_SMS_NUMBER_PLAN_TELEPHONY =
      1, /* CCITT E.164 and E.163, including ISDN plan */
...
typedef enum {
  ...
  RIL_CDMA_SMS_SUBADDRESS_TYPE_MAX32 =
      0x10000000 /* Force constant ENUM size in structures */
...
typedef struct {
   ...
   */
  /*it is constant for every short message which makes up a particular
   * concatenated short message*/
  unsigned char msg_ref;

  /*indicates the total number of short messages within the concatenated short
   message. The value shall start at 1 and remain constant for every short
   message which makes up the concatenated short message. if it is 0 then the
   receiving entity shall ignore the whole Information Element*/
  unsigned char total_sm;

  /*
   * it indicates the sequence number of a particular short message within the
   * concatenated short message. The value shall start at 1 and increment by one
   * for every short message sent within the concatenated short message. If the
   * value is zero or the value is greater than the value in octet 2 then the
   * receiving entity shall ignore the whole Information Element.
   */
...
typedef enum {
  ...
  RIL_CDMA_SMS_GW_MSG_WAITING_MAX32 =
      0x10000000 /* Force constant ENUM size in structures */
...
typedef enum {
  ...
  RIL_CDMA_SMS_GW_MSG_WAITING_KIND_MAX32 =
      0x10000000 /* Force constant ENUM size in structures */
...
typedef struct {
  ...
  /*it indicates the number of messages of the type specified in Octet 1
   * waiting.*/
...
typedef enum {
  ...
  RIL_CDMA_SMS_UDH_ALIGNMENT_MAX32 =
      0x10000000 /* Force constant ENUM size in structures */
...
typedef enum {
  ...
  RIL_CDMA_SMS_UDH_TEXT_COLOR_MAX32 =
      0x10000000 /* Force constant ENUM size in structures */
...
typedef struct {
  unsigned char position;
  unsigned char data[RIL_CDMA_SMS_UDH_ANIM_NUM_BITMAPS]
                    [RIL_CDMA_SMS_UDH_LARGE_BITMAP_SIZE];
...
typedef struct {
  unsigned char position;
  unsigned char data[RIL_CDMA_SMS_UDH_ANIM_NUM_BITMAPS]
                    [RIL_CDMA_SMS_UDH_SMALL_BITMAP_SIZE];
...
typedef struct {
  ...
  /* RIL_CDMA_SMS_UDH_EO_VCARD: See http://www.imc.org/pdi/vcard-21.doc for
   * payload */
...
typedef enum {
  ...
  RIL_CDMA_SMS_DELIVERY_STATUS_NETWORK_CONGESTION =
