...
#define GNSS_MULTIPATH_INDICATOR_NOT_PRESENT                                   \
  GNSS_MULTIPATH_INDICATIOR_NOT_PRESENT
...
#define GNSS_NAVIGATION_MESSAGE_TYPE_GPS_L1CA                                  \
  GNSS_NAVIGATION_MESSAGE_TYPE_GNSS_L1CA
#define GNSS_NAVIGATION_MESSAGE_TYPE_GPS_L2CNAV                                \
  GNSS_NAVIGATION_MESSAGE_TYPE_GNSS_L2CNAV
#define GNSS_NAVIGATION_MESSAGE_TYPE_GPS_L5CNAV                                \
  GNSS_NAVIGATION_MESSAGE_TYPE_GNSS_L5CNAV
#define GNSS_NAVIGATION_MESSAGE_TYPE_GPS_CNAV2                                 \
  GNSS_NAVIGATION_MESSAGE_TYPE_GNSS_CNAV2
 ...
 * navigation words, the status should be set to
 * NAV_MESSAGE_STATUS_PARITY_PASSED. But if a message is received with words
 * that failed parity check, but GPS is able to correct those words, the status
...
typedef struct {
   ...
   * GPS_POSITION_MODE_MS_BASED if GPS_POSITION_MODE_MS_ASSISTED is passed in,
   * and GPS_POSITION_MODE_MS_BASED is supported.
   */
...
typedef struct {
   ...
   * install. Returns: AGPS_CERTIFICATE_OPERATION_SUCCESS if the operation is
   * completed successfully AGPS_CERTIFICATE_ERROR_TOO_MANY_CERTIFICATES if the
   * HAL cannot store the number of certificates attempted to be installed, the
   * state of the certificates stored should remain the same as before on this
   * error case.
   *
   * IMPORTANT:
   ...
   */
  int (*install_certificates)(const DerEncodedCertificate *certificates,
                              size_t length);
   ...
   * provided. Returns: AGPS_CERTIFICATE_OPERATION_SUCCESS if the operation is
   ...
   * rest of them.
   */
  int (*revoke_certificates)(const Sha1CertificateFingerprint *fingerprints,
                             size_t length);
 ...
 */
typedef void (*gps_geofence_transition_callback)(int32_t geofence_id,
 ...
 * signals, it will call this callback with GPS_GEOFENCE_UNAVAILABLE parameter.
 *
 * Parameters:
 *  status - GPS_GEOFENCE_UNAVAILABLE or GPS_GEOFENCE_AVAILABLE.
 *  last_location - Last known location.
 */
typedef void (*gps_geofence_status_callback)(int32_t status,
 ...
 *          GPS_GEOFENCE_ERROR_TOO_MANY_GEOFENCES  - geofence limit has been
 * reached. GPS_GEOFENCE_ERROR_ID_EXISTS  - geofence with id already exists
 *          GPS_GEOFENCE_ERROR_INVALID_TRANSITION - the monitorTransition
 * contains an invalid transition GPS_GEOFENCE_ERROR_GENERIC - for other errors.
 */
 ...
 */
typedef void (*gps_geofence_remove_callback)(int32_t geofence_id,
                                             int32_t status);
 ...
 *          GPS_GEOFENCE_ERROR_GENERIC for others.
 */
typedef void (*gps_geofence_pause_callback)(int32_t geofence_id,
                                            int32_t status);
 ...
 *          GPS_GEOFENCE_ERROR_GENERIC for others.
 */
typedef void (*gps_geofence_resume_callback)(int32_t geofence_id,
                                             int32_t status);
...
typedef struct {
   ...
   * The reference receiver's time is specified by GpsData::clock::time_ns and
   * should be interpreted in the same way as indicated by GpsClock::type.
   *
   ...
   * GNSS_MEASUREMENT_STATE_UNKNOWN C/A code lock   : [ 0   1ms ]   :
   * GNSS_MEASUREMENT_STATE_CODE_LOCK is set Bit sync        : [ 0  20ms ]   :
   * GNSS_MEASUREMENT_STATE_BIT_SYNC is set Subframe sync   : [ 0    6s ]   :
   * GNSS_MEASUREMENT_STATE_SUBFRAME_SYNC is set TOW decoded     : [ 0 1week ]
   * : GNSS_MEASUREMENT_STATE_TOW_DECODED is set
   *
   *   Note well: if there is any ambiguity in integer millisecond,
   *   GNSS_MEASUREMENT_STATE_MSEC_AMBIGUOUS should be set accordingly, in the
   * 'state' field.
   *
   ...
   * valid range for this field can be: Searching       : [ 0       ]   :
   * GNSS_MEASUREMENT_STATE_UNKNOWN C/A code lock   : [ 0   1ms ]   :
   * GNSS_MEASUREMENT_STATE_CODE_LOCK is set Symbol sync     : [ 0  10ms ]   :
   * GNSS_MEASUREMENT_STATE_SYMBOL_SYNC is set Bit sync        : [ 0  20ms ]   :
   * GNSS_MEASUREMENT_STATE_BIT_SYNC is set String sync     : [ 0    2s ]   :
   * GNSS_MEASUREMENT_STATE_GLO_STRING_SYNC is set Time of day     : [ 0  1day ]
   * : GNSS_MEASUREMENT_STATE_GLO_TOD_DECODED is set
   *
   * For Beidou, this is:
   ...
   * valid range for this field can be: Searching    : [ 0       ] :
   * GNSS_MEASUREMENT_STATE_UNKNOWN C/A code lock: [ 0   1ms ] :
   * GNSS_MEASUREMENT_STATE_CODE_LOCK is set Bit sync (D2): [ 0   2ms ] :
   * GNSS_MEASUREMENT_STATE_BDS_D2_BIT_SYNC is set Bit sync (D1): [ 0  20ms ] :
   * GNSS_MEASUREMENT_STATE_BIT_SYNC is set Subframe (D2): [ 0  0.6s ] :
   * GNSS_MEASUREMENT_STATE_BDS_D2_SUBFRAME_SYNC is set Subframe (D1): [ 0    6s
   * ] : GNSS_MEASUREMENT_STATE_SUBFRAME_SYNC is set Time of week : [ 0 1week ]
   * : GNSS_MEASUREMENT_STATE_TOW_DECODED is set
   *
   * For Galileo, this is:
   ...
   * GNSS_MEASUREMENT_STATE_GAL_E1BC_CODE_LOCK is set E1C 2nd code lock: [ 0
   * 100ms ]   : GNSS_MEASUREMENT_STATE_GAL_E1C_2ND_CODE_LOCK is set
   *
   *     E1B page    : [ 0    2s ] : GNSS_MEASUREMENT_STATE_GAL_E1B_PAGE_SYNC is
   * set Time of week: [ 0 1week ] : GNSS_MEASUREMENT_STATE_TOW_DECODED is set
   *
   * For SBAS, this is:
   ...
   * provide GpsClock's 'drift' field as well (When providing the uncorrected
   ...
   * GPS_ADR_STATE_UNKNOWN. However, it is expected that the data is only
   * accurate when: 'accumulated delta range state' == GPS_ADR_STATE_VALID.
   */
  double accumulated_delta_range_m;
...
typedef struct {
   ...
   *    GPS_MEASUREMENT_ERROR_ALREADY_INIT - if a callback has already been
   * registered without a corresponding call to 'close'
   *    GPS_MEASUREMENT_ERROR_GENERIC - if any other error occurred, it is
...
typedef struct {
   ...
   * If this value is different from zero, 'data' must point to an array of the
   * same size. e.g. for L1 C/A the size of the sub-frame will be 40 bytes (10
   * words, 30 bits/word).
   *
   * This is a mandatory value.
   */
...
/**
 * The callback to report an available fragment of a GPS navigation messages
 * from the HAL.
 *
 * Parameters:
 *      message - The GPS navigation submessage/subframe representation.
 */
typedef void (*gnss_navigation_message_callback)(
...
typedef struct {
  ...
  /**
   * Initializes the interface and registers the callback routines with the HAL.
   * After a successful call to 'init' the HAL must begin to provide updates as
   * they become available.
   *
   * Status:
   *      GPS_NAVIGATION_MESSAGE_OPERATION_SUCCESS
   *      GPS_NAVIGATION_MESSAGE_ERROR_ALREADY_INIT - if a callback has already
   * been registered without a corresponding call to 'close'.
   *      GPS_NAVIGATION_MESSAGE_ERROR_GENERIC - if any other error occurred, it
   * is expected that the HAL will not generate any updates upon returning this
   * error code.
   */
...
typedef struct {
   ...
   *     config_data - a pointer to a char array which holds what usually is
   expected from file(/etc/gps.conf), i.e., a sequence of UTF8 strings separated
   by '\n'.
